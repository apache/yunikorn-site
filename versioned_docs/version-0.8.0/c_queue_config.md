...
The basis for the queue configuration is given in the [scheduler design document](e_scheduler_configuration.md#queue-configuration).

This document provides the generic queue configuration.
It references both the [Access control lists](c_acls.md) and [placement rule](c_placement_rules.md) documentation.
...
ACLs are documented in the [Access control lists](c_acls.md) document.
...
### Placement rules
The placement rules are defined and documented in the [placement rule](c_placement_rules.md) document.
