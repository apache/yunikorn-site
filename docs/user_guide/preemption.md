...
Included in the files is a Yunikorn Config that defines the queue configuration as follows:
...
| queue            | Resource before preemption occurs | Resource after preemption occurs       |
| ---------------- | --------------------------------- | -------------------------------------- |
...
| ----------------------- | ------------ | ------------------- |
| `no-guaranteed`         | 12           | x                   |
| `no-guaranteed.queue-1` | 10           | x                   |
| `no-guaranteed.queue-2` | 10           | x                   |
...
| queue                     | Resource before preemption occurs | Resource after preemption occurs |
| ------------------------- | --------------------------------- | -------------------------------- |
| `2-no-guaranteed.queue-1` | 10 (victim)                       | 6                                |
| `2-no-guaranteed.queue-2` | 2                                 | 6                                |

![preemption_no_guaranteed_case](../assets/preemption_no_guaranteed_case.png)

### Priority

In general, a high-priority pod can preempt a low-priority pod. You can set the priority by defining a [PriorityClass](https://kubernetes.io/docs/concepts/scheduling-eviction/pod-priority-preemption/) or by utilizing [queue priorities](priorities).
...
| queue                    | Max Resource | Guaranteed Resource | Types of priorities for Pods |
| ------------------------ | ------------ | ------------------- | ---------------------------- |
| `priority-class`         | 16           | x                   |                              |
| `priority-class.queue-1` | 8            | 3                   | allow-preemption             |
| `priority-class.queue-2` | 8            | 3                   | preemption-not-allow         |
| `priority-class.queue-3` | 8            | 3                   | allow-preemption             |
...
It is important to note that setting `yunikorn.apache.org/allow-preemption` is a strong recommendation but not a guarantee. For instance, DaemonSet can still preempt pods even if allow-preemption is set to false.

Furthermore, these rules can be broken if there are no other preemption options available. For example, in this scenario, if the application in `queue-1` has a higher priority than `queue-3`, `queue-2` will be preempted because an application can never preempt another application with a higher priority (law 6), and no other options are available.
...
In addition to utilizing the default PriorityClass in Kubernetes, you can also use [Yunikorn's queue priority](priorities).
